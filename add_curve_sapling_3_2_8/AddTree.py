...
from .settings_list import axes, branchmodes, handleList, leaftypes, shapeList, shapeList3, shapeList4, settings
...
class AddTree(bpy.types.Operator):
    ...
    leafShape: EnumProperty(name='Leaf Shape',
                            ...
                            items=(
                            ('hex', 'Hexagonal', '0'), ('rect', 'Rectangular', '1'), ('dFace', 'DupliFaces', '2'),
                            ('dVert', 'DupliVerts', '3')),
    ...
    def create_property_dict(self):
        ...
        # so we need something custom. This is it
        data = []
        for a, b in (self.as_keywords(ignore=(
                "chooseSet", "presetName", "limitImport", "do_update", "overwrite", "leafDupliObj"))).items():
