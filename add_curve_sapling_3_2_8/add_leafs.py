...
from add_curve_sapling_3_2_8.gen_leaf_mesh import gen_leaf_mesh
        ...
        for leaf_number, cp in enumerate(child_points):
            ...
            if leaf_settings.leafType == '4':
                ...
                for g in range(abs(leaf_settings.leaves)):
                    (vertTemp, faceTemp, normal, old_rotation) = gen_leaf_mesh(leaf_settings, cp.co, cp.quat, cp.offset,
                                                                         len(leaf_verts), old_rotation, leaf_number)
