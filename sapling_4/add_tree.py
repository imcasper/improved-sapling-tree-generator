...
def add_tree(props):
    ...
    #mesh branches
    if armature_settings.makeMesh:
        make_armature_mesh(armOb, armature_settings, cu, levelCount, resU, splineToBone,
                           treeMesh, treeObj, tree_settings)


def make_armature_mesh(armOb, armature_settings, cu, levelCount, resU, splineToBone,
                       treeMesh, treeObj, tree_settings):
    splineToBone1 = splineToBone
    splineToBone = [s[0] if len(s) > 1 else s for s in splineToBone1]
