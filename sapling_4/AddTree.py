...
#         default=False, update=no_update_tree)
    ...
    def draw(self, context):
        ...
        self.geometry_options(layout, context)
        # if self.chooseSet == '0':
    ...
    def geometry_options(self, layout, context):
        box = layout.box()
        box.label(text="Geometry:")
        row = box.row()
        # row.prop(self, 'bevel')
        row.prop(context.scene.test_sett, 'bevel')
        # row.prop(self, 'makeMesh')
        row = box.row()
        row.prop(context.scene.test_sett, 'bevelRes')
        row.prop(context.scene.test_sett, 'resU')
    ...
    def create_property_dict(self):
        ...
        # print("__getatt__.__getatt__\n", self.__getattribute__('test_property_group').__getattribute__('bevel'))

        # this!
        print("__getatt__.__getann__\n", self.__getattribute__('test_property_group').__getattribute__('__annotations__'))
        print("__getatt__.__getann__.valuse\n", self.__getattribute__('test_property_group').__getattribute__('__annotations__').values())
        # print("__getatt__.__getann__.__getatt\n", self.__getattribute__('test_property_group').__getattribute__('__annotations__').__getattribute__('bevel'))
        print("__getatt__.values\n", self.__getattribute__('test_property_group').values())
        print("__getatt__.keys\n", self.__getattribute__('test_property_group').keys())
        print("__getatt__.items\n", self.__getattribute__('test_property_group').items())

        print("__getatt__.__module__\n", self.__getattribute__('test_property_group').__module__)
        print("__getatt__.__dict__\n", self.__getattribute__('test_property_group').__dict__)
        print("__getatt__.__dir__\n", self.__getattribute__('test_property_group').__dir__())
        print("__getatt__.__doc__\n", self.__getattribute__('test_property_group').__doc__)
        # print("__getatt__.as_keywords\n", self.__getattribute__('test_property_group').as_keywords())
        print("___________________")
        prop = self.__getattribute__('test_property_group')
        ann = prop.__getattribute__('__annotations__')
        default_extractor(prop)
        for entry in ann:
            # print("is_set: ", prop.is_property_set(entry))
            print(entry, ": ", "ann.__getattribute__(entry)")
